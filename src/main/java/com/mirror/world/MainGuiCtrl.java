...
import static com.mirror.block.BlockNode.TYPE_COLORS;
...
public class MainGuiCtrl {
    ...
    private void initializeCodeArea() {
        ...
        dropLayer.setOnDragOver(event -> {
            if (event.getDragboard().hasContent(BLOCK_DEF_FORMAT) && event.getGestureSource() != null) {
                event.acceptTransferModes(TransferMode.COPY);
                event.consume();
                BlockDef def = (BlockDef) event.getDragboard().getContent(BLOCK_DEF_FORMAT);
                String color = TYPE_COLORS.getOrDefault(def.getType(), "#2196F3");
                showInsertPosition(event.getY(), color);
            }
        });
        ...
        dropLayer.setMouseTransparent(true);
        codeBlocks.getChildren().add(0, dropLayer);
        codeBlocks.addEventFilter(MouseEvent.MOUSE_PRESSED, event -> {
    ...
    private void showInsertPosition(double y, String color) {
        clearInsertIndicator();
        ...
        indicator.setStroke(Color.web(color));
    ...
    private void updateBlockPalette(String category) {
        ...
        blockMap.getOrDefault(category, List.of()).forEach(blockDef -> {
            ...
            String color = TYPE_COLORS.getOrDefault(blockDef.getType(), "#2196F3");
